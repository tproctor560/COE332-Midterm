...
def compute_location_astropy(sv):
    """
    Computes the latitude, longitude, and altitude of the ISS from the state vector.

    Args:
        sv (dict): A state vector dictionary containing 'X', 'Y', 'Z', and 'EPOCH'.

    Returns:
        tuple: Latitude, longitude, and altitude in km.
    """
    x = float(sv['X']['#text'])
    y = float(sv['Y']['#text'])
    z = float(sv['Z']['#text'])
    ...
    # Astropy conversion to Cartesian coordinates
...
def get_now_data():
    ...
    now = time.time()  # Get the current time in UNIX timestamp
    closest_epoch = None
